...
export const getPaginatePosts = async (page, limit = 3) => {
  const posts = await client.fetch(
    `*[_type=="blog"]{_createdAt, title, subtitle, 'category':category->{ title, 'picture':picture.asset->url }, date, 'image': cover_image.asset->url, 'slug':slug.current, 'publisher':publisher->{title, 'picture': picture.asset->url }}|order(date desc)[${page * limit}...${(page + 1) * limit}]`
  );
  return posts;
};
