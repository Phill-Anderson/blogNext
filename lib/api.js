...
export const getPaginatePosts = async (page, limit = 3) => {
  const posts = await client.fetch(
    `*[_type=="blog"]{_createdAt, title, subtitle, date, 'image': cover_image.asset->url, 'slug':slug.current, 'publisher':publisher->{title, 'picture': picture.asset->url }}|order(date desc)[${page * limit}...${(page + 1) * limit}]`
  );
  return posts;
};
...
// sanity-гаас ирсэн query -д өөрчлөлт орох үед realTime байдлаар тухайн өөрчлөлтйиг сонсох функц
export const listenPostUpdate = async (slug, fn) => {

  return await previewClient.listen(
    `*[_type=="blog" && slug.current==$slug]{_createdAt, title, subtitle, date, content[]{...,'asset': asset->} , cover_image, 'slug':slug.current, 'publisher':publisher->{title, 'picture': picture.asset->url,  }}`,
    { slug }
  ).subscribe(fn);
